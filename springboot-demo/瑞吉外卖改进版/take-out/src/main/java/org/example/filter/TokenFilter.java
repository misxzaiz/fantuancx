...
public class TokenFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        Map<String, Claim> tokenMap = JWTUtil.getJWT(authorization);
        log.info("jwt:{}",tokenMap);
        // 通过 id 在 redis 查询 token
        String token = (String) RedisUtils.get(tokenMap.get("id").toString());
        log.info("tokenMap.get(\"id\"):{}",tokenMap.get("id"));
        log.info("token:{}",token);
        // 如果token正确或者请求的是 /login，则继续执行后续操作
        if (uri.endsWith("/employee/login") || method.equals("OPTIONS")){
...
//            // 请求中的token是否有效
//            String jwt = JWTUtil.getJWT(authorization).toString();
//            log.info("jwt:{}",jwt);
